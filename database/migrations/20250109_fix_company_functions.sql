...
-- This handles the case where functions already exist with different signatures

-- Drop existing get_or_create_company function if it exists
DROP FUNCTION IF EXISTS get_or_create_company(TEXT);
DROP FUNCTION IF EXISTS get_or_create_company(TEXT, TEXT);
DROP FUNCTION IF EXISTS get_or_create_company(TEXT, TEXT, TEXT);

-- Drop existing search_companies_with_contacts function if it exists
DROP FUNCTION IF EXISTS search_companies_with_contacts(TEXT);
DROP FUNCTION IF EXISTS search_companies_with_contacts(TEXT, INTEGER);
...
COMMENT ON FUNCTION get_or_create_company(TEXT, TEXT, TEXT) IS 'Gets existing company by name or creates a new one if it doesn''t exist';
COMMENT ON FUNCTION search_companies_with_contacts(TEXT, INTEGER) IS 'Searches for companies and their contacts by name, email, or phone';
